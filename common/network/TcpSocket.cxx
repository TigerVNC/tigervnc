                             ...
                             int* port, int basePort)
{
  ...
  if (hi == nullptr)
    throw std::invalid_argument(_("NULL host specified"));
  ...
  if (hi[0] == '[') {
    ...
    if (hostEnd == nullptr)
      throw std::invalid_argument(_("Unmatched [ in hostname"));
  ...
  if (portStart == nullptr)
    *port = basePort;
    ...
    if (portStart[0] != ':')
      throw std::invalid_argument(_("Invalid port specified"));
...
const char* TcpSocket::getPeerAddress() {
  ...
  vlog.error(_("Unknown address family for socket"));
  return "";
}
                         ...
                         socklen_t listenaddrlen)
{
  ...
  if (bind(sock, &sa.u.sa, listenaddrlen) == -1) {
    int e = errorNumber;
    closesocket(sock);
    throw core::socket_error(_("Failed to bind socket"), e);
  }
...
TcpFilter::Pattern TcpFilter::parsePattern(const char* p) {
  ...
  if (parts.size() > 2)
    throw std::invalid_argument(_("Invalid filter specified"));
