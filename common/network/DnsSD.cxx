...
static AvahiClient* avahiClient = nullptr;
static AdaptivePoll* aPoll = nullptr;
...
static void publishService(Service* service)
{
  ...
  if (avahi_entry_group_is_empty(service->group)) {
    ...
    int e;
    e = avahi_entry_group_add_service(service->group, service->iface,
                                      service->protocol, (AvahiPublishFlags)0,
                                      service->name.c_str(), "_rfb._tcp", nullptr,
                                      nullptr, service->port, nullptr);
...
void dnssd::advertise(std::string name, std::list<SocketListener*>& listeners)
{
  ...
  if (avahi_client_get_state(avahiClient) == AVAHI_CLIENT_S_RUNNING)
    publishAll(); // Already running, publish right away
...
void dnssd::shutdown()
{
  ...
  destroyAvahiClient();
  if (aPoll) {
    aPoll->adapter->handleReadyWatch = nullptr;
    delete aPoll;
    aPoll = nullptr;
  }
  for (auto service : services)
