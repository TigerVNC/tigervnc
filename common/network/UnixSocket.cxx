...
UnixSocket::UnixSocket(const char *path)
{
  ...
  if (sock == -1)
    throw core::socket_error(_("Unable to create socket"), errno);
  ...
  if (result == -1)
    throw core::socket_error(_("Unable to connect to socket"), err);
...
UnixListener::UnixListener(const char *path, int mode)
{
  ...
  if (result < 0) {
    close(fd);
    throw core::socket_error(_("Failed to bind socket"), err);
  }
