...
Mutex::Mutex()
{
  ...
  if (ret != 0)
    throw core::posix_error("Failed to create mutex", ret);
#endif
}
...
void Mutex::lock()
{
  ...
  if (ret != 0)
    throw core::posix_error("Failed to lock mutex", ret);
#endif
}
...
void Mutex::unlock()
{
  ...
  if (ret != 0)
    throw core::posix_error("Failed to unlock mutex", ret);
#endif
}
...
Condition::Condition(Mutex* mutex_)
{
  ...
  if (ret != 0)
    throw core::posix_error("Failed to create condition variable", ret);
#endif
}
...
void Condition::wait()
{
  ...
  ret = SleepConditionVariableCS((CONDITION_VARIABLE*)systemCondition,
                                 ...
                                 INFINITE);
  if (!ret)
    throw core::win32_error("Failed to wait on condition variable", GetLastError());
#else
  int ret;
  ...
  if (ret != 0)
    throw core::posix_error("Failed to wait on condition variable", ret);
#endif
}
...
void Condition::signal()
{
  ...
  if (ret != 0)
    throw core::posix_error("Failed to signal condition variable", ret);
#endif
}
...
void Condition::broadcast()
{
  ...
  if (ret != 0)
    throw core::posix_error("Failed to broadcast condition variable", ret);
#endif
}
