...
VNCServerST::VNCServerST(const char* name_, SDesktop* desktop_)
  : blHosts(&blacklist), desktop(desktop_), desktopStarted(false),
    desktopStartupInProgress(false), blockCounter(0), pb(nullptr),
...
{
  assert(desktopStartupInProgress);
...
{
  if (!desktopStarted && !desktopStartupInProgress) {
    slog.debug("Starting desktop");
    desktopStartupInProgress = true;
    desktop->start();
  }
}
