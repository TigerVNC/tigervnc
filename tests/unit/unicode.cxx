...
TEST(Unicode, latin1ToUTF8)
{
  ...
  for (i = 0; i < ARRAY_SIZE(latin1utf8); i++) {
    ...
    EXPECT_EQ(rfb::latin1ToUTF8(latin1utf8[i].latin1), latin1utf8[i].utf8);
  }
}
...
TEST(Unicode, utf8ToLatin1)
{
  size_t i;
  for (i = 0; i < ARRAY_SIZE(latin1utf8); i++)
    EXPECT_EQ(rfb::utf8ToLatin1(latin1utf8[i].utf8), latin1utf8[i].latin1);
}
...
TEST(Unicode, utf16ToUTF8)
{
  ...
  for (i = 0; i < ARRAY_SIZE(utf8utf16); i++) {
    ...
    EXPECT_EQ(rfb::utf16ToUTF8(utf8utf16[i].utf16), utf8utf16[i].utf8);
  }
}
...
TEST(Unicode, utf8ToUTF16)
{
  ...
  for (i = 0; i < ARRAY_SIZE(utf8utf16); i++) {
    ...
    EXPECT_EQ(rfb::utf8ToUTF16(utf8utf16[i].utf8), utf8utf16[i].utf16);
  }
}
...
TEST(Unicode, isValidUTF8)
{
  ...
  for (i = 0; i < ARRAY_SIZE(validutf8); i++)
    EXPECT_TRUE(rfb::isValidUTF8(validutf8[i]));
  for (i = 0; i < ARRAY_SIZE(invalidutf8); i++)
    EXPECT_FALSE(rfb::isValidUTF8(invalidutf8[i]));
}
...
TEST(Unicode, isValidUTF16)
{
  ...
  for (i = 0; i < ARRAY_SIZE(validutf16); i++)
    EXPECT_TRUE(rfb::isValidUTF16(validutf16[i]));
  for (i = 0; i < ARRAY_SIZE(invalidutf16); i++)
    EXPECT_FALSE(rfb::isValidUTF16(invalidutf16[i]));
}
