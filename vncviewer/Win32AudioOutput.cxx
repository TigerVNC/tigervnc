...
bool Win32AudioOutput::openAndAllocateBuffer(size_t sizeInSeconds)
{
  if (!haveWO)
    return false;
  ...
  if (!openedWO) {
    ...
    // allocate buffer
    size_t buf_estim_size = sizeInSeconds * samplingFreq;
  ...
  for (HdrInSlist* hdr = spare_hdrs; hdr != NULL; hdr = hdr->next) {
    ...
    if (hdr->starvedWhenDone && (hdr->streamId == currentStreamId)) {
      ...
      if (now > hdr->starvationTimestamp) {
        ...
        if (delay_in_millisec > 1000)                                     // and clamp at one second
