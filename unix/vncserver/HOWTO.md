...
* The user has its own configuration in `$HOME/.config/tigervnc/config`.
...
password, then you have to make sure the `$HOME/.config/tigervnc` folder
created by `vncpasswd` have the correct *SELinux* context. You either
can delete this folder and recreate it again by creating the password one
moretime, or alternatively you can run:
```
$ restorecon -RFv /home/<USER>/.config/tigervnc
```
