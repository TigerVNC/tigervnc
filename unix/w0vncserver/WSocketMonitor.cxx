...
void WSocketMonitor::listen(rfb::VNCServerST* server)
{
  for (network::SocketListener* listener : listeners) {
    int fd;
    GIOChannel* channel;
...
int WSocketMonitor::sock_process(GIOChannel* source,
...
int WSocketMonitor::accept_connection(GIOChannel* source,
                                      ...
                                      void* data)
{
  ...
  fd = sock->getFd();
  channel = g_io_channel_unix_new(fd);
  g_io_add_watch(channel,(GIOCondition)G_IO_IN, sock_process, data_);
