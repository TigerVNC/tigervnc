...
static std::map<GSource*, GSocketSource*> sourceMap;
...
GSourceFuncs GSocketSource::sourceFuncs {
  .prepare = [](GSource* source, int* timeout) {
    try {
      return sourceMap.at(source)->prepare(timeout);
    } catch (std::out_of_range& e) {
      fatal_error("GSocketSource: %s", e.what());
  ...
  .dispatch = [](GSource* source, GSourceFunc callback, void* userData) {
    try {
      return sourceMap[source]->dispatch(callback, userData);
    } catch (std::out_of_range& e) {
      fatal_error("GSocketSource: %s", e.what());
      return G_SOURCE_REMOVE;
    }
