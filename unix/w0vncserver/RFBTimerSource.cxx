...
struct _RFBTimerSource {
  GSource base;
  RFBTimerSource* instance;
};

GSourceFuncs RFBTimerSource::sourceFuncs {
  .prepare = [](GSource* source, int* timeout) {
    return ((_RFBTimerSource*)(source))->instance->prepare(timeout);
  },
  .check = [](GSource* source) {
    return ((_RFBTimerSource*)(source))->instance->check();
  },
  .dispatch = [](GSource* source, GSourceFunc, void*) {
    return ((_RFBTimerSource*)(source))->instance->dispatch();
  },
  .finalize = nullptr,
...
{
  source = g_source_new(&sourceFuncs, sizeof(_RFBTimerSource));
}
