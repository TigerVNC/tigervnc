...
void RemoteDesktop::selectDevices()
{
  ...
  remoteDesktop->call("SelectDevices", params,
                           requestHandleToken.c_str(),
...
void RemoteDesktop::handleStart(GVariant *parameters)
{
  ...
  if (!g_variant_is_of_type(parameters, G_VARIANT_TYPE("(ua{sv})"))) {
    ...
    fatal_error("%s", core::format("Unexpected parameters type %s",
                                             ...
                                             GAsyncResult *res)
{
  ...
  if (!g_variant_is_of_type(response, G_VARIANT_TYPE("(h)"))) {
    ...
    fatal_error("%s", core::format("ScreenCast: invalid response type: %s, expected (h)",
...
bool RemoteDesktop::parseStreams(GVariant *streams_)
{
  ...
  stream = g_variant_get_child_value(streams_, 0);
