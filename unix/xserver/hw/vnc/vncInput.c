...
/* These are C headers, so safe to include */
#include <core/i18n.h>
...
static void vncKeysymKeyboardEvent(KeySym keysym, int down)
{
	...
	if (keycode == 0) {
		...
		if (keycode == 0) {
			LOG_ERROR(_("Failed to add new keysym XK_%s (0x%04x)"),
                                  KeySymName(keysym), keysym);
			return;
		}

		LOG_DEBUG("Added unknown keysym XK_%s (0x%04x) to keycode %d",
		...
		keycode = vncKeysymToKeycode(keysym, state, &new_state);
		if (keycode == 0) {
			LOG_ERROR(_("Could not map keysym XK_%s (0x%04x) to keycode"),
                                  KeySymName(keysym), keysym);
			return;
		}
	}
	...
	if (!(state & ShiftMask) && (new_state & ShiftMask)) {
		...
		if (shift_press == 0) {
			LOG_ERROR(_("Unable to find a modifier key for Shift"));
			return;
		}
		...
		if (shift_release_count == 0) {
			LOG_ERROR(_("Unable to find the modifier key(s) for releasing Shift"));
			return;
		}
	...
	if (!(state & level_three_mask) && (new_state & level_three_mask)) {
		...
		if (level_three_press == 0) {
			LOG_ERROR(_("Unable to find a modifier key for AltGr"));
			return;
		}
		...
		if (level_three_release_count == 0) {
			LOG_ERROR(_("Unable to find the modifier key(s) for releasing AltGr"));
			return;
		}
	...
	for (i = 0;i < 256;i++) {
		if (i == keycode)
			continue;
		if (pressedKeys[i] == keysym) {
			LOG_ERROR(_("Keysym XK_%s (0x%04x) has been generated by both keys %d and %d"),
