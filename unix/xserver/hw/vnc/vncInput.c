...
static void vncKeysymKeyboardEvent(KeySym keysym, int down)
{
	...
	if (!down) {
		 ...
		 * key presses.
		 */
		LOG_DEBUG("Unexpected release of keysym %s (0x%04x)",
	...
	if (keycode == 0) {
		...
		if (keycode == 0) {
			LOG_ERROR("Failure adding new keysym %s (0x%x)",
                                  KeySymName(keysym), keysym);
			return;
		}

		LOG_INFO("Added unknown keysym %s (0x%04x) to keycode %d",
		...
		keycode = vncKeysymToKeycode(keysym, state, &new_state);
		if (keycode == 0) {
			LOG_ERROR("Newly added keysym %s (0x%x) cannot be generated",
                                  KeySymName(keysym), keysym);
			return;
		}
	}
	...
	if (((state & ShiftMask) != (new_state & ShiftMask)) &&
		...
		LOG_DEBUG("Finding alternative to keysym %s (0x%x) to avoid fake shift for numpad",
                          KeySymName(keysym), keysym);
	...
	for (i = 0;i < 256;i++) {
		if (i == keycode)
			continue;
		if (pressedKeys[i] == keysym) {
			LOG_ERROR("Keysym %s (0x%04x) generated by both keys %d and %d",
