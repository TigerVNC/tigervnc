      ...
      if (!desktop[scr]) {
        ...
        if (scr == 0 && vncInetdSock != -1 && listeners.empty()) {
          ...
          if (!desktop[scr]->addClient(sock, false, false)) {
            delete sock;
            throw std::runtime_error(_("Failed to listen for VNC "
                                       "connections on inetd socket"));
          }
...
int vncConnectClient(const char *addr, int viewOnly)
{
  ...
  try {
    ...
    vlog.info(_("Reverse connection: %s:%d%s%s"), host.c_str(), port,
              viewOnly ? " " : "", viewOnly ? _(" (view only)") : "");
    if (!desktop[0]->addClient(sock, true, (bool)viewOnly)) {
      delete sock;
      throw std::runtime_error(_("Failed to add reverse connection socket"));
    }
  } catch (std::exception& e) {
